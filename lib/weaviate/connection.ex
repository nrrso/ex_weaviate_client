...
defmodule Weaviate.Connection do
  @moduledoc """
  Handle Tesla connections for Weaviate.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, Weaviate.Connection,
        base_url: "/v1",
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:weaviate, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - Weaviate 1.24.0 - elixir"
        )
      )
